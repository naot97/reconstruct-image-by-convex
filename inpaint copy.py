...
class Inpaint(ROFImg):
    ...
    def inpainting_smoothed_sq(self,x,g, a,b, l=0.5):
        #print(l)
        return (linalg.norm(self.Dx.dot(x))**2 + linalg.norm(self.Dy.dot(x))**2) + 0.5 *l* a.dot(linalg.norm(b - g.dot(x))**2)

    def inpainting_smoothed_sq_grad(self,x,g,a, b, l=0.5):
        return 2*(0.5*a*g*l*(g.dot(x) - b) + (self.Dx.T.dot(self.Dx.dot(x)) + self.Dy.T.dot(self.Dy.dot(x))))
    ...
    def inpainting_simulate(self):
        ...
        out = self.inpainting(ori,rows0,cols0,rows1,cols1,rows2,cols2)
